...
fun_get_predictors_labels <- function(table = NULL, vector_predictors_name = NULL, vector_predictors = NULL){
  ...
  if(length(vector_predictors_lsm)>0){
    ...
    vector_predictors_lsm <- vector_predictors_lsm %>%
      ...
      dplyr::arrange(code) %>%
      dplyr::mutate(group = ifelse(short_name %in% c("Marsh","Riparian forests","Still waters","Running waters","Rice"), "Landscape - wetlands","Landscape - other")) %>%
      dplyr::mutate(unit = ifelse(grepl("_3_12|_3_5",code),"pres","% of landscape"))
  }
...
fun_plot_tile_univ_spatial <- function(df, type, mod){
  ...
  df <- df %>% filter(p.value<=0.2) %>% 
    ...
    mutate(label = paste0(label," (",unit,")"))
  
  p <- ggplot(df, aes(buffer,label)) + 
    geom_tile(aes(fill = estimate), color = "white", show.legend = TRUE) + facet_grid(label_group~species, scales="free_y", space="free_y") +
    #ggtitle(paste(species, country, indicator, sep = " - "), subtitle = paste0("Model family : ", model_type)) + 
  ...
  if(type == "glmm"){
    p <- p + geom_text(aes(label = ifelse(is.na(estimate), "",paste0(round(estimate,2)," [",round(conf.low,2),"; ",round(conf.high,2),"] ",p.value2,"\n BIC = ",round(rsquared,2)))), size = 3) +
      #labs(caption = paste0("Format of the values displayed inside the cells : ",ifelse(indicator == "abundance","DDR","ODR")," [95% CI] / p-value \n Only significant results (pval <= 0.2) are displayed.")) + 
    #  scale_fill_discrete(na.value="transparent") 
      scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = ifelse(mod == "abundance", 0, 1), space = "Lab", name = ifelse(mod == "abundance","DDR","ODR"),
                          # limit = c(min(df$min), max(df$max)))
                          limit = c(min(df$min), ifelse(mod == "presence",max(df$max),1)), na.value = "red")
  }
...
fun_ccm_plot2 <- function(correlation_df, max_or_min, metric_name, var, filter_lags_ndays = 0, mod = "presence"){
  ...
  correlation_df <- correlation_df  %>%
    ...
    mutate(abs_corr = ifelse(p.value <= 0.2,abs_corr,NA)) %>%
    mutate(p.value2 = case_when(
      p.value <= 0.01 ~ "****",
      p.value >= 0.01 & p.value <= 0.03 ~  "***",
      p.value > 0.03 & p.value <= 0.05 ~  "**",
  ...
  ccm_plot <- ggplot(data = correlation_df, aes(time_lag_1, time_lag_2, fill = correlation)) +
    ...
    geom_tile(data = abs_corr , color = "deeppink3", size = 0.6, show.legend = FALSE, alpha = 0.9) +
    theme_minimal() + 
   ...
   # annotate("text", size = 3,x = min(correlation_df$time_lag_1), y = max(correlation_df$time_lag_2), vjust = "inward", hjust = "inward", label = paste0("r(0,0) = ", round(correlation_df$correlation[1],3),"\nr(",abs_corr$time_lag_1,",",abs_corr$time_lag_2,") = ",round(abs_corr$correlation,3))) +
    annotate("text", size = 3,x = min(correlation_df$time_lag_1)-0.5, y = max(correlation_df$time_lag_2)+0.5, vjust = "inward", hjust = "inward", label = paste0("r(",abs_corr$time_lag_1,",",abs_corr$time_lag_2,") = ",round(abs_corr$correlation,2)," [",round(abs_corr$conf.low,2),", ",round(abs_corr$conf.high,2),"] ",abs_corr$p.value2,"\n BIC = ",round(abs_corr$rsquared,2))) +
     coord_fixed() +
...
fun_plot_pdp <- function(modell, indicator, species){
  ...
  for(i in 1:nrow(imp)){
    ...
    p <-p %>% as_tibble(.) %>% dplyr::mutate(yhat = if_else(yhat<0,0,yhat))
...
model_univanalysis_results <- readRDS("/home/ptaconet/Bureau/data_analysis/model_results_univanalysis2.rds") %>%    #with bic weekly scale: model_results_univanalysis_glmm4.rds   # with bic and r2 and daily temporal resolution : model_results_univanalysis_glmm5.rds
  mutate(response_var = case_when(
    response_var == "ma_funestus_ss" ~ "An. funestus ss.",
    response_var == "ma_gambiae_ss" ~ "An. gambiae ss.",
    response_var == "ma_coluzzi" ~ "An. coluzzii")) %>%
  mutate(spatial_corrs_glmm = map(spatial_corrs_glmm, ~mutate(.,term = gsub("presence","",.$term)))) %>%
  mutate(spatial_corrs_glmm = map(spatial_corrs_glmm, ~fun_get_predictors_labels(table = ., vector_predictors_name = "term"))) %>%
  mutate(spatial_corrs_glmm = map(spatial_corrs_glmm, ~mutate(., buffer = word(gsub("_"," ",term), ifelse(unit %in% c("% of landscape","pres"), 4, 2))))) %>%
 ...
 univ_glmm_spatial <-  do.call(rbind.data.frame, model_univanalysis_results$spatial_corrs_glmm) %>% 
   filter(buffer %in% c(NA,250,500,1000,2000), term %in% c("WMD","WAC_2000","WLS_2000","lsm_c_pland_2000_3_12","lsm_c_pland_2000_3_3","lsm_c_pland_2000_3_2","lsm_c_pland_2000_3_4","lsm_c_pland_2000_3_9","lsm_c_pland_2000_3_1","lsm_c_pland_2000_3_7",
                                                    "WAC_1000","WLS_1000","lsm_c_pland_1000_3_12","lsm_c_pland_1000_3_3","lsm_c_pland_1000_3_2","lsm_c_pland_1000_3_4","lsm_c_pland_1000_3_9","lsm_c_pland_1000_3_1","lsm_c_pland_1000_3_7",
                                                    "WAC_500","WLS_500","lsm_c_pland_500_3_12","lsm_c_pland_500_3_3","lsm_c_pland_500_3_2","lsm_c_pland_500_3_4","lsm_c_pland_500_3_9","lsm_c_pland_500_3_1","lsm_c_pland_500_3_7",
                                                    "WAC_250","WLS_250","lsm_c_pland_250_3_12","lsm_c_pland_250_3_3","lsm_c_pland_250_3_2","lsm_c_pland_250_3_4","lsm_c_pland_250_3_9","lsm_c_pland_250_3_1","lsm_c_pland_250_3_7",
                                                    "WAC_100","WLS_100","lsm_c_pland_100_3_12","lsm_c_pland_100_3_3","lsm_c_pland_100_3_2","lsm_c_pland_100_3_4","lsm_c_pland_100_3_9","lsm_c_pland_100_3_1","lsm_c_pland_100_3_7")) %>%
   ...
   group_by(unit,label_group,indicator) %>%
   group_modify(~mutate(.,min = min(.$estimate, na.rm = T))) %>%
   group_modify(~mutate(.,max = max(.$estimate, na.rm = T))) %>%
   nest(-c(indicator,country))
  
 colnames_tempvar <- fun_get_temporal_preds_columns(0,42,c("RFD1F","TMAX1","TMIN1"),7,buffer_sizes=2000)
 
 colnames_tempvar <- fun_get_temporal_preds_columns(0,40,c("RFD1F","TMAX1","TMIN1","TAMP1"),1,buffer_sizes=2000)
 ...
 #  mutate(time_lag_1=ifelse(grepl("1",name),time_lag_1/7,time_lag_1),time_lag_2=ifelse(grepl("1",name),time_lag_2/7,time_lag_2),diff_lag=ifelse(grepl("1",name),diff_lag/7,diff_lag)) %>%
  ...
  plots_univ_glmm_temporal <- univ_glmm_temporal %>%
    arrange(desc(indicator),var,species) %>%
    mutate(plot_glmm = pmap(list(data, species, country, indicator), ~fun_ccm_plot2(..1, "max", "estimate", ..2, 0, ..4))) %>%
    nest(-c(var,indicator)) %>%
    mutate(plot_glmm = map2(data , indicator, ~wrap_plots(list(.x$plot_glmm[[2]],.x$plot_glmm[[3]],.x$plot_glmm[[1]])) + plot_annotation(title = .y) + plot_layout(guides = 'collect'))) %>% 
    dplyr::select(-data) %>%
    nest(-var) 
  
 p1 = wrap_plots(plots_univ_glmm_temporal$data[[1]]$plot_glmm, nrow = 2, ncol = 1) + plot_annotation(title = plots_univ_glmm_temporal$var[[1]])
 p2 = wrap_plots(plots_univ_glmm_temporal$data[[2]]$plot_glmm, nrow = 2, ncol = 1) + plot_annotation(title = plots_univ_glmm_temporal$var[[2]])
 p3 = wrap_plots(plots_univ_glmm_temporal$data[[3]]$plot_glmm, nrow = 2, ncol = 1) + plot_annotation(title = plots_univ_glmm_temporal$var[[3]])
...
#### spatial univariate
 ...
 plots_univ_glmm_spatial <- univ_glmm_spatial %>%
   ...
   mutate(plots_glmm = map2(data, indicator, ~fun_plot_tile_univ_spatial(df = ..1, type = "glmm", mod = ..2))) %>%
   dplyr::select(-data)


 plots_univ_glmm_spatial$plots_glmm[[1]] / plots_univ_glmm_temporal$data[[1]]$plot_glmm[[1]] / plots_univ_glmm_temporal$data[[2]]$plot_glmm[[2]]
