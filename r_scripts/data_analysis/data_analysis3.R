...
if(predictive_type == "notroi"){
  ...
  if(code_pays == "BF"){
    landcover_layers_to_keep <- c(4)
  } else if (code_pays == "CI"){
    landcover_layers_to_keep <- c(9)
  }
}
...
env_spatial_all <- load_spatial_data(code_pays, landcover_layers_to_keep, landcover_metrics_to_keep)
...
if(mod %in% c("presence","abundance")){
...
} else if(mod %in% c("physiological_resistance","exophagy","late_prec_aggressiveness")){
  ...
  if(mod=="late_prec_aggressiveness"){
    ...
    th_trmetrics_entomo_postedecapture <- th_trmetrics_entomo_postedecapture %>%
      mutate(resp_var == ifelse(heuredecapture))

  } else if (mod=="exophagy"){
    th_trmetrics_entomo_postedecapture <- th_trmetrics_entomo_postedecapture %>%
      mutate(resp_var == ifelse(postedecapture == "e",1,0))
  } else if (mod=="physiological_resistance"){
...
th_trmetrics_entomo_postedecapture <- th_trmetrics_entomo_postedecapture %>% 
  ...
  dplyr::select(idpostedecapture,idpointdecapture,int_ext,pointdecapture,pointdecapture2,codevillage,codepays,nummission,mean_date_mission,X_4326,Y_4326,X_32630,Y_32630,resp_var) %>%
...
corr_lsm1 <- corr_lsm1 %>% filter(pval <= 0.2)
...
if(nrow(corr_lsm1) > 1){
  ...
  p <- p %>%
    ...
    left_join(corr_lsm1 %>% dplyr::select(name,pval), by = c("stock1"="name")) %>%
    left_join(lco_metadata %>% dplyr::select(pixval,pixlabel,layer_id,priority2)) %>%
    dplyr::rename(priority = priority2) %>%
    dplyr::rename(layer_id1 = layer_id, pixval1 = pixval, pixlabel1 = pixlabel, priority1 = priority, correlation1 =  pval) %>%
    ...
    left_join(corr_lsm1 %>% dplyr::select(name,pval), by = c("stock2"="name")) %>%
    left_join(lco_metadata %>% dplyr::select(pixval,pixlabel,layer_id,priority2)) %>%
    dplyr::rename(priority = priority2) %>%
    dplyr::rename(layer_id2 = layer_id, pixval2 = pixval, pixlabel2 = pixlabel, priority2 = priority, correlation2 =  pval) %>%
...
corr_lsm <- corr_lsm %>%
    ...
    dplyr::select(name,pval,pixlabel,buffer) %>%
    rename(lab = pixlabel) %>%
...
corr_env_spatial <- corr_env_spatial %>% filter(pval<=0.2)
...
corr_env_nightcatch_postedecapture <- corr_env_nightcatch_postedecapture %>% filter(pval<=0.2)
if(nrow(corr_env_nightcatch_postedecapture)>0){
  ...
  corr_env_nightcatch_postedecapture <- corr_env_nightcatch_postedecapture %>% 
    ...
    dplyr::select(short_name,pval,name) %>%
    dplyr::rename(lab = short_name) %>%
...
corr_env_nightcatch <- corr_env_nightcatch %>% filter(pval<=0.2)
...
corr_env_static <- corr_env_static %>% filter(pval<=0.2)
...
if(model_type == "glmm"){
  ...
  th_trmetrics_entomo_postedecapture_mod2 <- th_trmetrics_entomo_postedecapture %>% mutate_at(results$ind_vars, ~scale(., center = TRUE, scale = FALSE)) %>% dplyr::select(c("resp_var","codevillage","pointdecapture2",var_to_keep))
  
  if(mod == "abundance"){
    th_mod <- buildglmmTMB(form, include = ~ VCM + (1|codevillage/pointdecapture2), data = th_trmetrics_entomo_postedecapture_mod2, family = truncated_nbinom2)
  } else if  (mod == "presence"){
    th_mod <- buildglmmTMB(form, include = ~ VCM + (1|codevillage/pointdecapture2), data = th_trmetrics_entomo_postedecapture_mod2, family = binomial(link = "logit"))
  }
...
if(model_type == "rf"){
  ...
  th_mod <- caret::train(x = th_trmetrics_entomo_postedecapture_mod1[,predictors_multivariate], y = th_trmetrics_entomo_postedecapture_mod1$resp_var,
                           method="ranger",tuneLength=5,metric=met,
                           trControl=tr_temporal,importance = "permutation")
  ...
  return(list(df = th_trmetrics_entomo_postedecapture, ccm_plots = ccm_plots, ts_plots = ts_plots, df_corr_univ = df_corr_univ, df_corr_multiv = df_corr_multiv, correlated_variables = p, model = th_mod, cv_lto = cv_lto))
