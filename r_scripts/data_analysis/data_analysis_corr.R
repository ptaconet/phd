...
fun_workflow_model <- function(response_var, 
  ...
  env_spatiotemporal <- load_spatiotemporal_data(vars = c("RFD1F","TMIN1","TMAX1","TAMP1","VNV8","VEV8","EVT8","SMO1"),
                                                 buffers = c(2000),
                                                 lag_time_window = c(0,42),
  ...
  env_spatiotemporal2 <- load_spatiotemporal_data(vars = c("VNV30","VMV30","WNW30","BRI30","WVV10","WVH10"),
                                                 buffers = c(100,250,500,1000,2000),
                                                 lag_time_window = c(0,42),
                                                 summarize_days_to_week = FALSE,
                                                 code_pays = code_pays,
                                                 entomo_csh_metadata_l1 = entomo_csh_metadata_l1)
  ...
  predictors <- setdiff(c(colnames(env_landcover), colnames(env_spatial),colnames_tempvar2, "WMD", "BDE"),"idpointdecapture")
  ...
  ## ccms
  time_vars <- c("RFD1F","TMIN1","TMAX1","TAMP1","VNV8","VEV8","EVT8","SMO1","WVV10","WVH10")
  ...
  colnames_tempvar1 <- fun_get_temporal_preds_columns(0,42,c("RFD1F","TMIN1","TMAX1","TAMP1","VNV8","VEV8","EVT8","SMO1"),7,th_trmetrics_entomo_postedecapture, 2000)
  colnames_tempvar3 <- fun_get_temporal_preds_columns(0,42,c("WVV10","WVH10"),7,th_trmetrics_entomo_postedecapture, c(100,250,500,1000,2000))
  colnames_tempvar3 <- setdiff(colnames_tempvar3,colnames_tempvar3[which(grepl("NA",colnames_tempvar3))])
  
  colnames_tempvar <- c(colnames_tempvar1,colnames_tempvar3)
...
saveRDS(model_results,"/home/ptaconet/Bureau/data_analysis/model_results_univanalysis_glmm4.rds")  # 
