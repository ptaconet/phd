...
fun_workflow_model <- function(response_var, 
  ...
  if(mod %in% c("presence","abundance","abundance_discrete")){
    ...
    if(code_pays == "BF"){ 
      lcid <- 3 
      spatial_vars_to_rm <- c("lsm_c_pland_2000_3_10","lsm_c_pland_2000_3_11","lsm_c_pland_2000_3_6","lsm_c_pland_2000_3_7","lsm_c_pland_2000_3_8","lsm_c_pland_2000_3_5") 
    } else if (code_pays == "CI"){ 
        lcid <- 8 
      }
    ...
    if(purpose == "explicative"){
       ...
       spatial_vars <- c(
                         colnames(th_trmetrics_entomo_postedecapture[which(grepl("lsm_c_pland",colnames(th_trmetrics_entomo_postedecapture)))]),
  ...
  cols_to_keep_timevar <- c("RFD1F_2000_0_30","TMIN1_2000_0_30","TMAX1_2000_0_30")
    
  spatial_vars2 <- setdiff(spatial_vars,c("lsm_c_pland_2000_3_12","lsm_c_pland_2000_3_5"))
  ...
  if(purpose == "predictive"){
    spatial_vars <- setdiff(spatial_vars,c("still_water_presence","running_water_presence","RFH","NMA","NMT","NMH"))
  } else {
    spatial_vars <- setdiff(spatial_vars,c("still_water_presence","running_water_presence"))
  }
  ...
  spatial_vars <- fun_multicol(th_trmetrics_entomo_postedecapture, spatial_vars)
   ...
   spatial_vars <- fun_feature_forward_selection(
    ...
    if(purpose == "explicative"){
      predictors <- c(cols_to_keep_timevar, spatial_vars, "VCM")
    } else if (purpose == "predictive"){
...
df_input_params_glmm <- df_input_params_glmm %>%
  ...
  add_row(response_var = "ma_funestus_ss", code_pays = "BF", mod = "abundance_discrete", purpose = "predictive") %>%
  add_row(response_var = "ma_gambiae_ss", code_pays = "BF", mod = "abundance_discrete", purpose = "predictive") %>%
  add_row(response_var = "ma_coluzzi", code_pays = "BF", mod = "abundance_discrete", purpose = "predictive")
...
saveRDS(model_results,"/home/ptaconet/Bureau/data_analysis/model_results_newnewanalysis_4.rds")
