...
path_to_db <- "data/react_db/react_db_light.gpkg"
...
fun_map <- function(codepayss,tit){
 ...
 roi <- sf::st_read(path_to_db,"contexte_frontieresreact") %>% 
  ...
  m <- ggmap(myMap) + geom_point(aes(x = X, y = Y, shape = intervention), data = villages, size = 2)  + facet_grid(.~nummission) + labs(colour = "Vector control tool") + theme(legend.position = ifelse(codepayss=="BF", 'bottom','none'),axis.title.x=element_blank(),axis.title.y=element_blank()) + ggtitle(tit)
...
m1/m2 + plot_annotation(title = 'Vector control tool implemented in each village and entomological survey')
...
dates_add_vc <- dbReadTable(react_gpkg, 'recensement_villages_l1') %>% dplyr::select(-geom) %>%
  filter(intervention %in% c("IRS","IVM","IEC","Larvicide")) %>%
   ...
   summarise(date = mean(as.Date(date_debut_interv))) %>%
    as_tibble() %>%
  mutate(Surveys = "Implementation of complementary VC tool", survey_num = NA)
...
dates_to_plot <- rbind(dates_entomo,dates_recensement,dates_cpthumain,dates_epi_active,dates_llin,dates_add_vc) %>%
  mutate(Surveys = forcats::fct_relevel(Surveys, c("Human + animal census","Anopheles behaviour survey","Human behaviour survey","Epidemio - active","LLIN distribution","Implementation of complementary VC tool"))) %>%
  filter(Surveys %in% c("Human + animal census","Anopheles behaviour survey","Human behaviour survey","Implementation of complementary VC tool","LLIN distribution")) %>% 
  mutate(position = case_when(codepays=="BF" & Surveys %in% c("Human + animal census","Anopheles behaviour survey","Human behaviour survey","Epidemio - active") ~ 0.5,
                              codepays=="CI" & Surveys %in% c("Human + animal census","Anopheles behaviour survey","Human behaviour survey","Epidemio - active") ~ -0.5,
                              codepays=="BF" & Surveys %in% c("Implementation of complementary VC tool","LLIN distribution") ~ 0.3,
                              codepays=="CI" & Surveys %in% c("Implementation of complementary VC tool","LLIN distribution") ~ -0.3,
                              )) %>%
...
plot_lus <- ggplot(lus , aes(x=survey, y = perc_users_llin, fill = name)) +
  geom_col(fill = "grey") + 
   ...
   theme_bw() + 
   labs(title = "% of LLIN users for each study area and human behaviour survey", 
           caption = "Values were averaged over all the sampled villages. \nError bars indicate the mean +- sd for all the study villages for the considered survey")
...
ggplot(data = HBI) +
  stat_ecdf(aes(hintmaison, colour = as.factor(survey)), geom = "step") + 
  facet_wrap(.~codepays)
...
crops_bf <- dbReadTable(react_gpkg, 'env_landcover') %>% 
  ...
  summarise(val = mean(val)) %>%
  mutate(codepays="BF",var="crop",short_name="Surface of crops", unit="% of landscape\nin a 2 km buffer area")
...
oth_spatial2 <- oth_spatial %>%
  ...
  filter(short_name %in% c("Human population (person)",
                           "Shortest dist. to stream (m.)",
                           "Degree of clustering of the households (NA)",
                           "Distance to the edge of the village (meters)",
                           "Surface of crops (% of landscape\nin a 2 km buffer area)")) %>%
...
th_env_nightcatch_postedecapture <- th_env_nightcatch_postedecapture %>%
  ...
  mutate(entomo_mission="entomological mission")
...
plot_microclimate_bf <- ggplot(th_env_nightcatch_postedecapture %>% filter(codepays=="BF"), aes(x=date, y = vale)) + 
  ...
  scale_x_datetime(labels = date_format("%H"),
                     date_breaks = "2 hours") +
  labs(y = NULL) + 
  labs(x = "hour",title = "Study area : Diébougou (BF)") + 
  theme_bw() + 
...
plot_microclimate_ci <- ggplot(th_env_nightcatch_postedecapture %>% filter(codepays=="IC"), aes(x=date, y = vale)) + 
  ...
  scale_x_datetime(labels = date_format("%H"),
                     date_breaks = "2 hours") +
  labs(y = NULL) + 
  labs(x = "hour",title = "Study area : Korhogo (IC)") + 
  theme_bw() + 
  theme(strip.text.y = element_text(size = 7))

p <- plot_microclimate_ci / plot_microclimate_bf + plot_annotation(
  title = 'Micro-climatic conditions during each entomological survey',
  caption = 'Ribbons indicate the mean +- sd for all the collection points of the study area'
) 
